...
def run_report(settings: dict) -> None:
    ...
    _apply_truth(apply_ambetter_truth, "Ambetter",
                 lambda s: f"+{s['added_from_portal']} added, "
                           ...
                           f"({s['protected_new_sales']} new sales protected, {s.get('absent_kept', 0)} kept: absent, {s.get('reenrolled_kept', 0)} kept: re-enrolled)", "ambetter")
    _apply_truth(apply_oscar_truth, "Oscar",
                 lambda s: f"+{s['added_from_portal']} added, "
                           f"{s['cancelled_inactive'] + s['cancelled_dropped']} marked cancelled "
                           f"({s['protected_new_sales']} new sales protected, {s.get('absent_kept', 0)} kept: absent, {s.get('reenrolled_kept', 0)} kept: re-enrolled)", "oscar")
    _apply_truth(apply_uhc_truth, "UHC",
                 lambda s: f"+{s['added_policies']} added, "
                           ...
                           f"({s['protected_new_sales']} new sales protected, {s.get('absent_kept', 0)} kept: absent, {s.get('reenrolled_kept', 0)} kept: re-enrolled)", "united|uhc")
    _apply_truth(apply_anthem_truth, "Anthem",
                 lambda s: f"+{s['added_policies']} added, "
                           f"{s['cancelled_lapsed'] + s['cancelled_dropped']} marked cancelled "
                           f"({s['protected_new_sales']} new sales protected, {s.get('absent_kept', 0)} kept: absent, {s.get('reenrolled_kept', 0)} kept: re-enrolled)", "anthem")
    _apply_truth(apply_cigna_truth, "Cigna",
                 lambda s: f"+{s['added_from_portal']} added, "
                           f"{s['cancelled_inactive'] + s['cancelled_dropped']} marked cancelled "
                           f"({s['protected_new_sales']} new sales protected, {s.get('absent_kept', 0)} kept: absent, {s.get('reenrolled_kept', 0)} kept: re-enrolled)", "cigna")
